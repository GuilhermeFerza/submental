...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState<Stats>({
    totalMixtapes: 0,
    nextEvent: { title: "Carregando...", date: "--" }
  });
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="border-4 border-white p-6 flex flex-col justify-between min-h-[160px]">
                <h3 className="text-lg font-bold uppercase">Sets Ativos</h3>
                <p className="text-6xl font-black">{stats.totalMixtapes}</p>
              </div>

              <div className="border-4 border-white p-6 flex flex-col justify-between min-h-[160px]">
                <h3 className="text-lg font-bold uppercase">Próximo Evento</h3>
                <div>
                  <p className="font-bold text-xl uppercase truncate">{stats.nextEvent.title}</p>
                  <p className="text-sm opacity-70">
                    {stats.nextEvent.date !== "--" 
                      ? new Date(`${stats.nextEvent.date}T00:00:00`).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' }).toUpperCase()
                      : "--"}
                  </p>
                </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-black text-white font-mono flex flex-col md:flex-row">
      <aside className="w-full md:w-64 border-b-4 md:border-b-0 md:border-r-4 border-white p-6 flex flex-col justify-between">
        <div>
          <Link to="/" className="block w-full text-3xl font-black uppercase tracking-widest mb-8 border-b-4 border-white pb-4" target="blank">
            Submental
          </Link>
