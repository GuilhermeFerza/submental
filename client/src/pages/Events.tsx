...
export function Events() {
    ...
    if (loading) return <p className="text-white bg-black min-h-screen p-8 font-mono">Loading...</p>;
                    ...
                    <section>
                        ...
                        <ul className="flex flex-col">
                            {upcomingEvents.map((evento) => (
                                <li 
                                    className="flex justify-between items-center py-6 border-b border-white/30 hover:bg-white hover:text-black transition-colors cursor-pointer px-4 group" 
                                    key={evento.id}
                                    onClick={() => setSelectedEvent(evento)}
                                >   
                                    <div className="flex items-center gap-8 w-full">
                                        <span className="text-2xl md:text-4xl font-bold w-24 md:w-32">
                    ...
                    <section>
                        ...
                        <ul className="flex flex-col text-zinc-500">
                            {pastEvents.map((evento) => (
                                <li 
                                    className="flex justify-between items-center py-4 border-b border-zinc-800 hover:text-zinc-300 transition-colors cursor-pointer px-4" 
                                    key={evento.id}
                                    onClick={() => setSelectedEvent(evento)}
                                >   
                                    <div className="flex items-center gap-8 w-full">
                                        <span className="text-xl font-bold w-24">
                                            {evento.date ? evento.date.split('-').slice(1, 3).reverse().join('/') : ''}
                                        </span>
